...
/// Windows-safe socket read: uses ws2_32.recv() instead of posix.read() (NtReadFile)
/// to avoid error.Unexpected from unmapped Win32 error codes on socket handles.
fn socketRead(socket: posix.socket_t, buf: []u8) posix.ReadError!usize {
    if (comptime builtin.os.tag == .windows) {
        const ws2 = std.os.windows.ws2_32;
        const rc = ws2.recv(socket, buf.ptr, @intCast(buf.len), 0);
        if (rc == ws2.SOCKET_ERROR) {
            const err = ws2.WSAGetLastError();
            return switch (err) {
                ws2.WinsockError.WSAEWOULDBLOCK => error.WouldBlock,
                ws2.WinsockError.WSAECONNRESET => error.ConnectionResetByPeer,
                ws2.WinsockError.WSAECONNABORTED => error.ConnectionResetByPeer,
                ws2.WinsockError.WSAESHUTDOWN => error.BrokenPipe,
                ws2.WinsockError.WSAETIMEDOUT => error.WouldBlock,
...
/// Windows-safe socket write: uses ws2_32.send() instead of posix.write().
fn socketWrite(socket: posix.socket_t, data: []const u8) posix.WriteError!usize {
    if (comptime builtin.os.tag == .windows) {
        const ws2 = std.os.windows.ws2_32;
        const rc = ws2.send(socket, data.ptr, @intCast(data.len), 0);
        if (rc == ws2.SOCKET_ERROR) {
            const err = ws2.WSAGetLastError();
            return switch (err) {
                ws2.WinsockError.WSAEWOULDBLOCK => error.WouldBlock,
        ...
        return posix.write(socket, data);
    }
}
...
fn _handleHandshake(comptime H: type, worker: anytype, hc: *HandlerConn(H), ctx: anytype) !struct { bool, bool } {
    ...
    log.warn("({f}) [PATCH] attempting socketRead on fd", .{conn.address});
    const n = socketRead(hc.socket, buf[len..]) catch |err| {
        switch (err) {
            ...
            else => log.warn("({f}) handshake error reading from socket (patched): {}", .{ conn.address, err }),
        }
        return .{ false, false };
    };
